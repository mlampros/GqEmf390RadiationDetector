...
fn list_serial_ports(settings: SerialPortSettings) {
    if let Ok(ports) = serialport::available_ports() {
        ...
        for p in ports {
            match serialport::open_with_settings(p.port_name.as_str(), &settings) {
                Ok(mut port) => { 
                    w("<GETVER>>", &mut port);
                    let o = r(&mut port, '\r' as u8).unwrap();                    
                    println!("  {}: {}", p.port_name, o);
                }
                Err(_e) => { }
            };            
        }
        ...
        match serialport::open_with_settings(&port_name, &settings) {
            Ok(mut port) => {
                if matches.is_present("format") {
                    ...
                    let _ = download_data(&mut port, output_file_name, address, max_amount, influx);                     
                } else if matches.is_present("power") {
                    let action = matches.value_of("power").unwrap();
                    if action == "on" {
                        let _  = exec_command("<POWERON>>", "ef", &mut port, true, 'm');
                    } else if action == "off" {
                        let _ = exec_command("<POWEROFF>>", "ef", &mut port, true, 'm');
                    } else {
                        print!("Allowed values are only 'on' and 'off' which respectively turn the unit on and off")
                    }
